...
deque<Event> InputHandler::input;
...
void InputHandler::handleKey(int key, int action, int mods)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		e.setType(EventType::MOVE_FORWARD);
		input.push_back(e);
	}
	else if (key == GLFW_KEY_A && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
		cout << "client move left" << endl;
		e.setType(EventType::MOVE_LEFT);
		input.push_back(e);
	}
	else if (key == GLFW_KEY_S && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
		cout << "client move backward" << endl;
		e.setType(EventType::MOVE_BACKWARD);
		input.push_back(e);
	}
	else if (key == GLFW_KEY_D && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
		cout << "client move right" << endl;
		e.setType(EventType::MOVE_RIGHT);
		input.push_back(e);
	}
